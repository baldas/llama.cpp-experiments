...
struct llama_model_params {
    ...
    // on-demand loading settings (Linux only, requires userfaultfd)
    size_t ondemand_max_resident;  // max bytes to keep resident in memory (0 = unlimited)
    ...
    bool use_ondemand;        // use on-demand loading (Linux only, uses userfaultfd)
    bool ondemand_lru_evict;  // enable LRU eviction when memory is constrained
};
