...
struct llama_model::impl {
    ...
    // on-demand loaders for models using on-demand loading
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    for (auto & [buft, ctx_ptr] : ctx_map) {
        ...
        if ((ml.use_mmap || ml.use_ondemand) && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
    ...
    // Transfer on-demand loaders to model for lifetime management
    if (ml.use_ondemand) {
        for (auto & loader : ml.ondemand_loaders) {
            pimpl->ondemand_loaders.emplace_back(std::move(loader));
        }
    }
