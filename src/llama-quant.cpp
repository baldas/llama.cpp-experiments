...
static void llama_model_quantize_impl(const std::string &                 fname_inp,
    ...
    llama_model_loader ml(fname_inp, splits, use_mmap, /*use_ondemand*/ false, /*check_tensors*/ true, kv_overrides,
                          nullptr);
    ...
    if (qs.n_attention_wv != 0 && !is_clip_model) {
        ...
        const int32_t n_layer_recr =
            std::count(model.hparams.recurrent_layer_arr.begin(), model.hparams.recurrent_layer_arr.end(), true);

        LLAMA_LOG_INFO("%s: n_layer_attn = %d, n_layer_recr = %d, pruned_attention_w = %d\n", __func__, n_layer_attn,
                       n_layer_recr, pruned_attention_w);

        GGML_ASSERT((qs.n_attention_wv == n_layer_attn - pruned_attention_w - n_layer_recr) &&
