...
llama_model_loader::llama_model_loader(const std::string &                      fname,
                                       ...
                                       const llama_ondemand_params *            ondemand_params) {
    int trace = 0;
    ...
    this->use_ondemand  = use_ondemand;
    this->check_tensors = check_tensors;

    // On-demand loading is mutually exclusive with mmap
    if (use_ondemand) {
#if LLAMA_ONDEMAND_SUPPORTED
        if (use_mmap) {
            LLAMA_LOG_WARN("%s: on-demand loading enabled, disabling mmap\n", __func__);
            this->use_mmap = false;
        }
#else
        LLAMA_LOG_WARN("%s: on-demand loading not supported on this platform, falling back to mmap\n", __func__);
        this->use_ondemand = false;
#endif
    }

    // Store on-demand params for later use in init_mappings
    if (ondemand_params) {
        this->ondemand_params = *ondemand_params;
    } else {
        this->ondemand_params = llama_ondemand_default_params();
    }
...
void llama_model_loader::init_mappings(bool prefetch, llama_mlocks * mlock_mmaps) {
    // If on-demand loading is enabled, use that instead of mmap
    if (use_ondemand) {
        init_mappings_ondemand(ondemand_params);
        return;
    }
...
void llama_model_loader::init_mappings_ondemand(const llama_ondemand_params & params) {
#if LLAMA_ONDEMAND_SUPPORTED
    if (!use_ondemand) {
        return;
    }

    LLAMA_LOG_INFO("%s: initializing on-demand loading for %zu file(s)\n", __func__, files.size());

    ondemand_loaders.reserve(files.size());
    mmaps_used.reserve(files.size());

    for (const auto & file : files) {
        auto loader = std::make_unique<llama_ondemand_loader>(file.get(), file->size(), params);
        if (!loader->start()) {
            throw std::runtime_error(format("%s: failed to start on-demand loader", __func__));
        }
        mmaps_used.emplace_back(loader->get_size(), 0);
        ondemand_loaders.emplace_back(std::move(loader));
    }
...
#else
    (void) params;
    LLAMA_LOG_WARN("%s: on-demand loading not supported on this platform\n", __func__);
#endif
}
...
void llama_model_loader::get_mapping_range(size_t *       first,
    ...
    // Support both regular mmap and on-demand loading
    if (use_ondemand) {
#if LLAMA_ONDEMAND_SUPPORTED
        GGML_ASSERT(!ondemand_loaders.empty());
        const auto & loader = ondemand_loaders.at(idx);

        *first = loader->get_size();
        *last  = 0;
        *addr  = loader->get_base_addr();
        ...
        return;
#else
        GGML_ABORT("on-demand loading not supported");
#endif
    }
...
void llama_model_loader::load_data_for(struct ggml_tensor * cur) const {
    ...
    if (use_ondemand) {
#if LLAMA_ONDEMAND_SUPPORTED
        // On-demand loading: point tensor data to the on-demand region
        // Data will be loaded automatically via page faults when accessed
        const auto & loader = ondemand_loaders.at(w.idx);
        if (cur->data == nullptr) {
            cur->data = (uint8_t *) loader->get_base_addr() + w.offs;
        } else {
            // Copy from on-demand region to tensor buffer
            memcpy(cur->data, (uint8_t *) loader->get_base_addr() + w.offs, ggml_nbytes(cur));
        }
#else
        GGML_ABORT("on-demand loading not supported");
#endif
...
bool llama_model_loader::load_all_data(struct ggml_context *   ctx,
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (use_ondemand) {
            ...
            // On-demand loading: point tensor data to the on-demand region
            // Data will be loaded automatically via page faults when accessed
            const auto &          loader       = ondemand_loaders.at(weight->idx);
            ggml_backend_buffer_t buf_ondemand = nullptr;
            if (bufs.count(weight->idx)) {
                buf_ondemand = bufs.at(weight->idx);
            }
            uint8_t * data = (uint8_t *) loader->get_base_addr() + weight->offs;

            // Note: tensor validation is deferred since data isn't loaded yet
            // It will be loaded on first access via page fault

            GGML_ASSERT(buf_ondemand || cur->data);
            if (buf_ondemand && cur->data == nullptr) {
                ggml_backend_tensor_alloc(buf_ondemand, cur, data);
